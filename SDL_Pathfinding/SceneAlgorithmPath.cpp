...
SceneAlgorithmPath::SceneAlgorithmPath()
{
	draw_grid = false;
...
void SceneAlgorithmPath::update(float dtime, SDL_Event* event)
{
	...
	case SDL_KEYDOWN:
		...
		if (event->key.keysym.scancode == SDL_SCANCODE_8)
		{
			...
			vector<Vector2D> path = algorithmBFS->CalculatePath();
...
const char* SceneAlgorithmPath::getTitle()
{
	return "SDL Path Finding :: PathFinding Mouse Demo";
}

void SceneAlgorithmPath::drawMaze()
{
...
void SceneAlgorithmPath::drawCoin()
{
...
bool SceneAlgorithmPath::loadTextures(char* filename_bg, char* filename_coin)
{
